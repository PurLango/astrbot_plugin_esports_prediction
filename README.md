...
4. 需要兑换功能时，再按下一节添加第一个兑换物；抽奖、生日、日期口令和平台专属兑换均可之后逐项调整
...
- `private_only` 默认建议开启，避免敏感内容发到群里；发放内容可以公开时可关闭
...
1. 发送 `/兑换列表` 查看名称、价格、库存和是否需要私聊
2. 选择兑换物后发送 `/兑换 群福利`；也支持输入能够唯一匹配该名称的片段
3. 若列表提示“请私聊兑换”，就在与机器人的私聊中发送同一条兑换指令
4. 兑换成功后会扣除积分并发放一份内容；管理员补充新的 `contents` 项即可增加库存
...
- `exchange_items.*.private_only`：是否仅允许私聊领取敏感内容；关闭后可在群聊直接兑换
...
- 兑换记录仅展示兑换物、用户 ID、消耗与时间；发放内容不会被复制到兑换记录中
